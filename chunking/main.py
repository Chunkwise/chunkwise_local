...
class InputItem(ChunkingConfig):
    text: str
...
@app.post("/chunks")
def chunk(item: InputItem) -> list[Chunk]:
    chunker = create_chunker_from_config(item)
    return get_chunks(item.text, chunker) 
